	...
	<div>
					...
					<div class="_box _b_radious3 _padd20">  
						<div class="_1card_top _mar_b20">
							<div class="_1card_top_left">
								<p class="_1card_top_title">Course table</p>
						...
						<div class="_table_responsive">
							<Table class="_table700" border :columns="columns1" :data="data1">
								...
								<template slot-scope="{ row  }" slot="description">
                                    ...
                                    <Button  type="success" class="_button_black"   @click="showVideos(row.id)"> Show Videos</Button>
								</template>
                                ...
                                <template slot-scope="{ row, index }" slot="action">
                                     <nuxt-link :to="`/course/${row.id}`"><Button  type="info"  class="_button_black"  size="small">EDIT</Button></nuxt-link>
	    ...
	    <Modal v-model="deleteModal" width="600">
              ...
              <div  v-if="data1[editIndex]" style="text-align:center">
                  Are you sure you want to delete {{ data1[editIndex].title }} Course?
              </div>
              <div slot="footer">
        ...
        <Modal  v-model="videoModal" width="500" :mask-closable="false"   :closable="false">
            <p slot="header" style="color:#19be6b;text-align:center">
                <span> Videos </span>
            </p>
            ...
            <div slot="footer">
                <Button type="success" @click="videoModal=false" long >Close</Button>
            </div>
        </Modal>
        <Modal v-model="videoVisibleModal" width="620" :footer-hide="true" :mask-closable="false">
            <div slot="close">
                <Button   @click="closeVideo" long ><i class="fas fa-times">X</i></Button>
                <!-- <span style="font-size: 20px;color: #a8a8a8dd;" @click="closeVideo" >X
            ...
            >
                <video  :src="`${videoVisibleItem.source}`"  :type="`video/${videoVisibleItem.extType}`"  ></video>
            </vue-plyr>
      </Modal>
	</div>
</template>
...
export default {
	...
	methods:{
        ...
        // console.log(item)
         ...
         showVideos(item){
            var index = this.data1.findIndex(x => x.id === item);
            if(this.data1[index].data){
                this.contentList = this.data1[index].data
                this.contentList = JSON.parse(this.contentList);
            }
        ...
        async remove(){ 
            ...
            const res = await this.callApi('post',`blog/deleteBlog`, { id:this.data1[this.editIndex].id }  )
             if(res.status == 200){
                ...
                this.s('Great!','Course has been deleted successfully!')
                this.loading = false
        ...
        async getAllData(){
                this.loading = true
                const res = await this.callApi('get', `course/getAllCourse?page=${this.page}&filterField=${this.filterField}`)
                if (res.status == 200) { 
